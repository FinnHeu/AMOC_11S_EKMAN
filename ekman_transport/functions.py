...
def angle_to_pure_east(U: np.ndarray=np.ndarray, V: np.ndarray=np.ndarray) -> np.ndarray:
    ...
    for i in range(len(U)):
        if np.logical_and(U[i] == 0, V[i] == 0):
            ...
            alpha[i] = np.arccos((real_wind[0,i]*assumed_wind[0] + real_wind[1,i]*assumed_wind[1])
                                  /(np.sqrt(real_wind[0,i]**2 + real_wind[1,i]**2) * np.sqrt(assumed_wind[0]**2 + assumed_wind[1]**2)))
...
# large_and_pond_1981
def large_and_pond_1981_ext(U: np.ndarray = np.ndarray, extend_ranges=True
) -> np.ndarray:
...
def yelland_and_taylor_1996_ext(U: np.ndarray = np.ndarray, extend_ranges=True) -> np.ndarray:
...
def Ekman_Transport(U: np.ndarray = np.ndarray,
    ...
    ekman_transp_u, ekman_transp_v = ekman_transport_calc(tau=tau,ekman_layer_depth_array=ekman_layer_depth_array,f_coriolis_array=f_coriolis_array,alpha=alpha)
